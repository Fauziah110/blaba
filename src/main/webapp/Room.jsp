...
<script>
	...
	function validateRoomForm(formId) {
		const form = document.getElementById(formId);
		const priceField = form.querySelector('input[name="roomPrice"]');
		const priceValue = priceField.value.trim();

		const numberOnlyPattern = /^\d+(\.\d+)?$/;

		if (!numberOnlyPattern.test(priceValue)) {
			alert("Please enter a valid numeric price (e.g., 123 or 123.45)");
			priceField.focus();
			return false;
		}
		return true;
...
<%
    ...
    if ("invalidpassword".equals(errorParam)) {
%>
<script>
...
%>
	...
	<table>
		...
		<tbody>
			...
			<tr>
				...
				<td><%=rs.getBigDecimal("ROOMPRICE").toPlainString()%></td>
		...
		<div id="addRoomModal" class="modal">
			<div class="modal-content">
				...
				<h2>Add Room</h2>
				<form id="addRoomForm" action="Room.jsp" method="post" onsubmit="return validateRoomForm('addRoomForm')">
	...
	<select id="staffInCharge" name="staffInCharge">
		<%
		conn = ConnectionManager.getConnection();
	...
	<button type="submit">Add Room</button>
		...
		if ("POST".equalsIgnoreCase(request.getMethod())) {
		    ...
		    String roomPriceParam = request.getParameter("roomPrice");

		    if (!roomPriceParam.matches("\\d+(\\.\\d+)?")) {
		        out.println("<script>alert('Please enter a valid numeric price (e.g., 123.45)'); history.back();</script>");
		        return;
		    }

		    double roomPrice = Double.parseDouble(roomPriceParam);
		...
		<div id="editRoomModal" class="modal">
			<div class="modal-content">
				...
				<form id="editRoomForm" action="EditRoomController" method="post" onsubmit="return validateRoomForm('editRoomForm')">
	<!-- Hidden fields -->
	<input type="hidden" id="isEdit" name="isEdit" value="false">
	<input type="hidden" id="originalRoomId" name="originalRoomId" value="">

	<label for="roomId">No Room:</label>
	<input type="text" id="roomId" name="roomId" readonly><br>
...
<input type="text" id="roomPrice" name="roomPrice" required
       pattern="^\d+(\.\d+)?$" title="Please enter a valid numeric price (e.g., 123 or 123.45)"><br>
	...
	<select id="staffInCharge" name="staffInCharge">
		...
		try {
			...
			while (rs3.next()) {
		%>
		<option value="<%=rs3.getInt("staffid")%>"><%=rs3.getString("staffname")%></option>
		<%
		}
		} catch (SQLException e) {
			e.printStackTrace();
...
<script>
    window.onload = function () {
        <% if ("invalidPassword".equals(deleteError)) { %>
            alert("Invalid staff password.");
        <% } else if ("deleteFailed".equals(deleteError)) { %>
            alert("Failed to delete the room.");
        <% } else if ("sqlError".equals(deleteError)) { %>
            alert("A database error occurred while deleting.");
        <% } %>
    };
</script>
<%
